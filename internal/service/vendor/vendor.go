...
func (vo *VendorObj) UploadVendorImagesV1(vendorCode string, imageFor string,
	file multipart.File, fileHeader *multipart.FileHeader, vendorId, vehicleID string) (*dtos.UploadResponse, error) {
	...
	if imageFor == "" {
		return nil, errors.New("imageFor should not be empty. imageFor:rc_expiry_doc/insurance_doc/pucc_expiry_doc/np_expire_doc/fitness_expiry_doc/tax_expiry_doc/mp_expire_doc/pancard_img/bank_passbook_or_cheque_img/tds_declaration")
	}
	imageTypes := [...]string{
		"rc_expiry_doc",
		"insurance_doc",
	...
	imageName := fmt.Sprintf("%v_%v.%v", imageFor, vendorCode, extension[lengthExt-1])
	vendorCodeImageFullPath := filepath.Join(fullPath, imageName)
	vo.l.Info("vendorCodeImageFullPath: ", imageFor, vendorCodeImageFullPath)
